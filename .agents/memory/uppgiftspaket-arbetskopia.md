...
- **Varje ny skapandeväg för uppgifter MÅSTE fylla paketet:** paketfyllnad kastar vid fel (skapandet avbryts högljutt), aldrig tyst svälj. Storage-hooks täcker de vanliga vägarna; routes som gör direkta inserts måste fylla själva utan `.catch(→undefined)`.
- **Artikel-snapshot:** paketets `artikel` fryser register-/radvärden (id/nr/namn/pris/kostnad/produktionstid/debiteringsmodell) vid skapandet. Första artikelbärande orderrad/assignment-artikel stämplar snapshoten via CAS-update (skriv endast om `artikelId` saknas i jsonb) — skydd mot samtidiga första-rader. Propagering och backfill måste SPREADA/bevara befintliga snapshotfält och bara uppdatera koderna — aldrig bygga om dem från dagens register/rader. Registeruppslag sker ALDRIG efter skapandet.
...
- **Propagering trunkeras ALDRIG:** subträds-expansionen är uttömmande (loud warn vid mycket stora subträd, chunkade IN-listor). En tyst cap bryter kravet "alla öppna/framtida uppgifter".
