...
**Why:** Task #1243 required the client to be a pure API wrapper so it can be tested/mocked independently of business rules, and so idempotency/logging logic has one home instead of being duplicated across call sites (routes, invoice queue, credit flow).
