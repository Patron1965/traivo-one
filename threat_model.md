...
- Important production-scope notes from this scan:
  ...
  - Ordinary tenant membership still provides overly broad access to some planner/control-plane routes. The current confirmed gaps include urgent-job dispatch endpoints, `POST /api/work-orders/:id/send-sms`, `GET /api/work-orders/:id/communications`, the earlier-registered unguarded `POST /api/work-orders/:id/auto-eta-sms`, `POST /api/route/send-to-mobile`, and `POST /api/ai/suggest-placement`.
  - On bypassed mobile surfaces, the strongest current risk is horizontal misuse within the tenant rather than the earlier suspected fail-open tenant fallback. The current confirmed gaps are object-level: `PATCH /api/mobile/objects/:id/location` keeps write access alive for any worker with a historical assignment to that object, and `POST /api/mobile/customer-change-requests` lets any authenticated mobile worker target arbitrary same-tenant objects. Earlier concerns about immediate-active team invites and caller-controlled completion attribution appear fixed in the currently inspected code.
  - Several production routes live in `server/routes/mobile/misc.ts` despite using ordinary `/api/*` web paths rather than mobile bearer auth. The earlier raw-ID checklist/quick-action concern was not re-confirmed in the latest review, but raw storage helpers in `server/storage.ts` such as `getWorkOrder`, `updateWorkOrder`, `updateObject`, and checklist-template accessors remain high-priority review points whenever a route does not independently verify tenant or object ownership.
  - Portal auth and storage paths require continued review even when the high-level flow looks hardened. The current scan confirmed public tenant-roster exposure and timing-based customer email enumeration in portal login bootstrap, while the earlier portal-module disable-state bypass and signed-URL object-scope bypass appear fixed. Portal upload confirmation still allows authenticated customers to create permanent orphaned files that bypass cleanup and can be abused for storage-cost exhaustion.
