...
- Important production-scope notes from this scan:
  ...
  - The global API response logger in `server/index.ts` logs the first 200 characters of JSON responses in production. Any endpoint that returns bearer-style credentials or share links must be treated as log-sensitive unless response-body logging is removed or redacted.
  - Ordinary tenant membership currently provides overly broad access to several operational control-plane routes. In the latest review this included customer-communications controls (`/api/eta-notification/*`, `/api/work-orders/:id/auto-eta-sms`, `/api/protocols/:id/send-to-customer`, `/api/ai/communications/*`), shared AI planning/apply routes under `/api/ai/*`, IoT telemetry reads under `GET /api/iot/*`, plus previously noted public-support/staff-support routes such as `/api/qr-code-links*`, `/api/public-issue-reports*`, `/api/staff/portal-messages*`, and `GET /api/visit-confirmations`. These surfaces require explicit server-side role enforcement beyond the default `/api` tenant gate.
  - On bypassed mobile surfaces, root-path or alias handlers require extra scrutiny. The current production review found cross-tenant exposure risks in `GET /resource_profile_assignments` and in mobile team membership flows under `server/routes/mobile/team.ts` because those handlers trust caller-supplied resource identifiers or membership rows without enforcing tenant ownership.
