...
- Important production-scope notes from this scan:
  ...
  - Ordinary tenant membership currently provides overly broad access to several operational control-plane routes. In the latest review this included AI/customer-communications endpoints such as `POST /api/ai/eta-check-delays`, `GET /api/ai/communications*`, tool-based data access at `POST /api/ai/field-assistant`, and planner-grade AI analysis/apply routes such as `POST /api/ai/planning-suggestions`, `POST /api/ai/assisted-plan`, `GET /api/ai/setup-insights`, `GET /api/ai/predictive-planning`, and `POST /api/ai/apply-setup-updates`. These surfaces require explicit server-side role enforcement beyond the default `/api` tenant gate.
