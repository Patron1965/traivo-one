...
- **Orderkoncept System (Phase 1):** Scenario-based order automation with three modes: Avrop (on-demand), Schema (recurring with delivery schedule + time windows), and Abonnemang (subscription with monthly fee per unit). Features include: delivery schedule builder with month/week/weekday/time window entries, rolling month generation (configurable 1-12 months ahead), subscription calculation with metadata-driven unit counts (e.g. antal_kärl), change detection for subscription objects (new/removed/quantity changes) with approval workflow, preview endpoint for dry-run execution, and `subscription_changes` table for tracking changes. DB tables: `order_concepts` (extended with scenario, deliverySchedule, rollingMonths, monthlyFee, billingFrequency, contractLockMonths, subscriptionMetadataField), `subscription_changes`. API: `/api/order-concepts/:id/preview`, `/api/order-concepts/:id/run-rolling`, `/api/order-concepts/:id/subscription-calc`, `/api/order-concepts/:id/detect-changes`, `/api/subscription-changes`.
