...
- **Smart Resursförslag (Kluster-tillhörighet):** Automatic resource suggestions based on cluster membership. WeekPlanner drag highlights cluster-matching resources with emerald green glow. AssignDialog sorts resources by cluster match with "Rekommenderad" badge (Sparkles icon) and no-match warning when no resource serves the job's cluster. Backend `GET /api/clusters/resource-match` endpoint for scored resource suggestions. AI auto-schedule cluster weight boosted to +30.
