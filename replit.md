...
### Planner Search Filters (F3 - implementerat)
Sparade sökmönster för planeraren via `planner_search_filters` (tenant + scope `personal|shared`, valfri team-koppling, `filter_criteria` JSONB). Endpoints: `GET/POST/PATCH/DELETE /api/planner-search-filters` — alla bakom `isAuthenticated + requireTenantWithFallback + requirePlannerAccess`. `teamId` valideras mot tenant vid create/patch. UI: `/planner-search-filters` (admin-meny + CommandPalette under "Planering"). Lagring/route: `server/storage.ts` (`getPlannerSearchFilters` filtrerar shared+egna), `server/routes/plannerRoutes.ts` (slutet av filen). Personliga mönster är osynliga för andra användare även inom samma tenant.

### Article Components — BOM (F4 - implementerat)
Strukturartiklar (TILG100) bryts ner i komponentrader (TILG201, TILG202) via `article_components` (unik på `parent_article_id + child_article_id`, `quantity` real, `is_mandatory` bool, `sort_order`, `notes`). Endpoints: `GET /api/articles/:parentId/components` (auth), `POST/PATCH/DELETE` (alla `requireAdmin`). Validering: childId ≠ parentId (no self-ref), child måste finnas i tenant, 23505-fångst → ConflictError "redan i strukturen", PATCH strippar `tenantId`/`parentArticleId`/`childArticleId`. UI: `/article-components` (admin-meny "Artikelkomponenter (BOM)" + CommandPalette under "System"). Parent-väljaren visar endast `isStructure=true`-artiklar; komponent-kandidater filtrerar bort strukturartiklar (förhindrar nesting). Lagring: `server/storage.ts` (5 metoder, alla tenant-skoppade), routes i `server/routes/configRoutes.ts` direkt efter DELETE /api/articles/:id. **Standalone-sida** valdes (i stället för tab i 1778-rader ArticlesPage) för att eliminera regressionsrisk.

### Frozen WO Snapshot + Invoice Recalculation (F5 - implementerat)
Per-WO snapshot (`work_orders.frozen_unit/frozen_quantity/frozen_unit_price/frozen_unit_cost/frozen_unit_time` + `metadata_snapshot`) tas via `POST /api/work-orders/:id/freeze` (requireAdmin). Summerar workOrderLines (qty * resolvedPrice/Cost/Minutes) till per-enhet-värden, snapshotar object.metadata. Idempotent — refusar omfrysning om `frozenUnitPrice != null` om inte `?force=true`. `POST /api/work-orders/:id/recalculate` (requireAdmin) jämför nuvarande WO-radvärde mot `frozenUnitPrice * frozenQuantity`, skriver delta till `invoice_recalculation_log` (endast om |delta| > 0.001). `GET /api/invoice-recalculation-log` (auth, paginerad, filter `?workOrderId`) listar alla omräkningar. UI: `/invoice-recalculation-log` (admin-meny + CommandPalette under "System") med stats-kort (antal/total nettoförändring/senaste) + tabell med reason-badge, WO-länk, prev/new/delta i SEK och affectedPeriods. Befintliga 36 534 WO med `frozen_*=NULL` är opåverkade — fakturas via `cachedValue/work_order_lines` som idag.
