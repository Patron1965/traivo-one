...
- **Annual Planning (Årsplanering):** Annual goal tracking per customer/object with `annual_goals` table. CRUD endpoints at `/api/annual-goals`. Auto-generation from subscriptions and order concepts via `POST /api/annual-goals/generate-from-subscriptions`. Frontend at `/annual-planning` with KPI summary cards, progress bars, forecast icons (green/yellow/red), filter/search/sort, warning tab for behind/at-risk goals. **AI-driven besöksfördelning:** "AI-fördelning" tab with wizard dialog for scope/period selection, `POST /api/annual-planning/ai-distribute` analyzes goals and proposes optimal monthly distribution using OpenAI, comparison bar chart (Recharts) showing current vs proposed distribution per goal with pagination, `POST /api/annual-planning/apply-distribution` creates work orders according to approved proposal. Respects season restrictions, resource capacity, and existing completed orders.
...
- **Predictive Maintenance (`/predictive-maintenance`):** AI-driven predictive maintenance using IoT signal history. Analyzes signal patterns per object to forecast next service date with confidence scoring (high/medium/low). Baseline algorithm computes average intervals between actionable signals; OpenAI enrichment for pattern recognition. KPI cards (urgent/upcoming/later/confidence), filterable forecast table, and one-click work order creation from predictions. Cached in `predictive_forecasts` table.
