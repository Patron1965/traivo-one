...
- **Tenant Configuration:** Dedicated `/tenant-config` page for tenant setup with 4 tabs: Företagsinfo (company details, org number, contact, industry), Artiklar & Exekveringskoder (inline execution code assignment on articles), Prislistor (overview with link to full management), Resurser & Behörigheter (toggle execution codes per resource). Includes setup completion progress indicator.
