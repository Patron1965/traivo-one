...
- **Async Route Optimization:** `useRouteOptimization` hook with async job polling (submit → poll status → get result). Server proxy endpoints: `POST /optimize-route`, `GET /optimize-route/:jobId/status`, `GET /optimize-route/:jobId/result`. Mock mode returns immediate completion. WebSocket events `route:optimized` and `route:reoptimizing` for real-time route updates. MapScreen optimization banner during processing.
- **Adaptive GPS Tracking:** `useGpsTracking` with speed-adaptive interval (15s moving / 60s idle, threshold 2 m/s). Offline position buffer (100 positions in AsyncStorage) with batch flush via `POST /position/batch`. `driver_location_history` table for historical tracking. `currentOrderId` tracking for ML correlation.
- **Enhanced Route Feedback (ML-ready):** Expanded reason options (10 categories incl. wrong_order, road_closed, better_known_route). Auto-capture of actual vs planned metrics via `/route-metrics/today`. `optimization_job_id` linkage for ML pipeline correlation. Forward to traivo-one `/api/optimization/feedback` in live mode.
