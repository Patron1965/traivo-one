...
- **Offline Capabilities:** Offline map data caching (route polylines + order data in AsyncStorage, 24h TTL), offline sync for outbox operations (interval adapts to offline mode setting), and robust handling of network disconnections. Sync status indicator (green/yellow/red dot) on HomeScreen.
...
- **Communication:** Real-time updates via WebSocket (Socket.io) for order changes and notifications. Extended Traivo WS events: `job_assigned`, `job_updated`, `job_cancelled`, `schedule_changed`, `priority_changed`, `anomaly_alert`, `position_update`, `ping/pong` keepalive. WebSocket bridge (`server/websocketBridge.ts`) forwards events from Traivo One to mobile clients in live mode, with exponential backoff reconnection. Bridge status available via `GET /api/health`. Position updates from mobile clients are forwarded upstream.
- **Server Mode Detection:** `GET /api/mobile/server-mode` returns `{ mode: 'mock'|'live' }`. MockIndicator component shows yellow "Testläge" banner when running in mock mode. All mock JSON object responses include `_mock: true`. Per-request logging shows `[MOCK]` or `[LIVE]` prefix. Live-mode proxy errors return 503 with Swedish error messages (no silent fallback to mock data).
...
- **HomeScreen Enhancements:** Task type summary badge (e.g. "3 kärl, 2 containers"), 10-minute warning card before scheduled jobs, full weather display row (temp/description/wind/precipitation), next order card with address + navigate-to-Maps deep link + estimated minutes. Notification bell icon with unread badge in header.
- **Notification Center (Aviseringar):** NotificationsScreen with grouped list (today/yesterday/earlier), 8 notification types (order_assigned, status_change, team_invite, schedule_change, deviation_reviewed, material_update, sign_off_complete, system) with color-coded icons and type badges. Tap to navigate to related order/team. "Mark all read" header button. Unread count badge on HomeScreen bell icon. GET `/api/mobile/notifications`, POST `/api/mobile/notifications/:id/read`, POST `/api/mobile/notifications/read-all`. Accessible from HomeScreen header and ProfileScreen menu.
...
- **Settings System:** Global settings via `client/lib/settings.ts` with `useSettings()` hook. Settings: GPS tracking (synced with `useGpsTracking`), push notifications, haptic feedback (global via `client/lib/haptics.ts`), offline mode (adjusts sync interval), dark mode (UI ready, theming pending), map app preference (Google/Apple/Waze via `client/lib/navigation-links.ts`). "Clear cache" button clears offline/route/order caches. AsyncStorage key: `driver_core_settings`.
- **Error Resilience:** `ScreenErrorBoundary` component wraps MapScreen, AIAssistantScreen, and InspectionScreen with per-screen retry UI. Coordinate validation filters invalid lat/lng before map rendering. Global `ErrorBoundary` branded as "Traivo Go".
- **Distance API (R2):** `POST /api/mobile/distance` replaces haversine with road-network driving times. `fetchDrivingDistance()` in `client/lib/travel-time.ts`. HomeScreen next-order card shows real driving time + km. 15min stale, 5min refetch. Batch endpoint: `POST /api/mobile/distance/batch`.
- **Disruption Triggers (R3):** `useDisruptionMonitor` hook checks active orders every 5min for >150% estimated duration → triggers `POST /api/mobile/disruptions/trigger/delay`. Early completion trigger when last order done with >45min slack → `POST /api/mobile/disruptions/trigger/early-completion`. Sick leave button in ProfileScreen → `POST /api/mobile/disruptions/trigger/resource-unavailable`. Mock disruption events stored in `MOCK_DISRUPTIONS` array.
- **Break Stops (R4):** `GET /api/mobile/break-config` returns tenant break settings. MapScreen legend inserts break stop (coffee icon) at computed position between 11:00-13:00 window. Break stops are informational only (not actionable).
