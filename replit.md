...
- **Field Worker Task Dependency View:** Enhanced SimpleFieldApp with dependency-aware job cards showing Lock/Unlock badges for dependent tasks. Locked jobs (with uncompleted dependencies) display red-tinted muted styling with toast warnings. Field worker endpoints: `GET /api/field-worker/tasks?date=X&resourceId=Y` returns tasks with dependency info, `POST /api/field-worker/tasks/:id/start`, `POST /api/field-worker/tasks/:id/complete` (auto-marks dependency instances as completed), `POST /api/field-worker/tasks/:id/update-metadata`.
- **Inspection & Metadata System:** Structured inspection checklist integrated into SimpleFieldApp job detail view. 6 inspection categories (Dörr, Lås, Fönster, Belysning, Golv, Ventilation) with OK/Varning/Fel status, predefined issue badges, and comment fields. DB table: `inspection_metadata` with inspectionType, status, issues (JSONB), comment, photoUrls, inspectedBy, inspectedAt. API: `GET/POST /api/inspection-metadata`, `GET /api/inspection-metadata/search`.
- **Inspection Search Page:** Admin page at `/inspections` for searching and filtering inspection results by type and status. Features: statistics cards (total/ok/warning/error counts), data table with checkboxes for multi-select, CSV export with Swedish encoding, and batch action order creation from inspection results. Accessible via sidebar under "Besiktning".
- **Invoice Preview/Generation:** Backend endpoint `GET /api/invoice-preview` generates invoice previews from completed work orders grouped by customer. Supports 4 billing types (per_task, per_room, per_area, monthly), metadata on invoice headers and line items, 25% VAT calculation, and waitForAll logic. Filter by orderConceptId, customerId, fromDate, toDate.
