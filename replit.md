...
- **Execution Codes (C8 - Utförandekoder):** Many-to-many execution code system mapping resource capabilities to task requirements. Resources have `executionCodes` array, articles and work orders have `executionCode` field. Auto-plan enforces code matching. Frontend shows execution code icons (🚛🧹🔧⚙️🔍🚚👷) on job cards and provides execution code filter in orderstock sidebar.
