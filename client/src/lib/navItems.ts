...
export function getGrunddataItems(t: (key: string, fallback: string) => string, tl?: (key: string) => string, lang?: string): NavItem[] {
  ...
  return [
    ...
    { title: "Utförarregister", url: "/utforarregister", icon: UserCheck, description: "Samlad vy: personer, fordon/utrustning och team som en enhet — med kostnadsställe och projekt" },
    ...
    { title: "Utförandekoder", url: "/execution-codes", icon: Workflow, description: "Hantera utförandekoder (tjänstetyper) per organisation", adminOnly: true },
    { title: "Ikoner", url: "/icons", icon: Shapes, description: "Hantera ikonbibliotek för artiklar per organisation", adminOnly: true },
