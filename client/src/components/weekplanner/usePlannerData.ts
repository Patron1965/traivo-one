...
export function usePlannerData() {
  ...
  const handleCarryOver = useCallback(async () => {
    ...
    } catch (error) { toast({ title: "Kunde inte flytta jobb", description: (error as Error).message, variant: "destructive" }); }
  }, [toast]);
