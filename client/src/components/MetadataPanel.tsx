        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-sm font-medium truncate">
              {metadataDisplayName(metadata.katalog)}
            </span>
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2">
          <Icon className="h-4 w-4 text-muted-foreground flex-shrink-0" />
          <span className="text-sm font-medium truncate">
            {metadataDisplayName(first.katalog)}
          </span>
          <Badge
            variant="secondary"
  ...
  return (
    <Card>
      ...
      <Dialog open={!!editingMetadata} onOpenChange={(open) => !open && setEditingMetadata(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Redigera {editingMetadata ? metadataDisplayName(editingMetadata.katalog) : ''}</DialogTitle>
          </DialogHeader>
          {editingMetadata && (
            <EditMetadataForm
