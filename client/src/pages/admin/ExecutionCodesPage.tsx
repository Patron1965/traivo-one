...
export default function ExecutionCodesPage() {
  const { toast } = useToast();
  const { user } = useAuth();
  const isAdmin = user?.role === "admin" || user?.role === "owner";

  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<ExecutionCodeDefinition | null>(null);
  const [formData, setFormData] = useState<ExecutionCodeFormData>(emptyForm);
  const [keyTouched, setKeyTouched] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<ExecutionCodeDefinition | null>(null);

  const { data: codes = [], isLoading, isError, refetch } = useQuery<ExecutionCodeDefinition[]>({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setDialogOpen(false);
      resetForm();
      toast({ title: "Utförandekod skapad" });
    },
    onError: (err: Error) => {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setDialogOpen(false);
      resetForm();
      toast({ title: "Utförandekod uppdaterad" });
    },
    onError: (err: Error) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await apiRequest("DELETE", `/api/execution-codes/${id}`);
      return res.json();
    },
  ...
  if (!isAdmin) {
    return (
      <div className="p-6">
        <PageHeader title="Utförandekoder" description="Hantera utförandekoder per organisation" icon={Workflow} />
        <Card className="mt-6">
          <CardContent className="flex items-center gap-2 py-8 text-muted-foreground" data-testid="text-access-denied">
            <Lock className="h-4 w-4" />
            Endast administratörer kan hantera utförandekoder.
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        title="Utförandekoder"
        description="Definiera vilka utförandekoder (tjänstetyper) som kan kopplas till artiklar, resurser och resursprofiler."
        icon={Workflow}
      >
        <Button onClick={openCreate} data-testid="button-create-execution-code">
      ...
      <Card>
        ...
        <CardContent>
          <QueryState isLoading={isLoading} isError={isError} onRetry={refetch} isEmpty={codes.length === 0} emptyDescription="Inga utförandekoder ännu.">
            <Table>
              <TableHeader>
                <TableRow>
      ...
      <Dialog open={dialogOpen} onOpenChange={(o) => { if (!o) resetForm(); setDialogOpen(o); }}>
        <DialogContent>
          <form onSubmit={handleSubmit}>
            <DialogHeader>
              <DialogTitle>{editing ? "Redigera utförandekod" : "Ny utförandekod"}</DialogTitle>
              <DialogDescription>
                {editing
            ...
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  maxLength={80}
                  required
                  data-testid="input-execution-code-label"
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  disabled={!!editing}
                  required
                  data-testid="input-execution-code-key"
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  data-testid="input-execution-code-sort"
                />
              </div>
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => { resetForm(); setDialogOpen(false); }} data-testid="button-cancel-execution-code">
                Avbryt
              </Button>
              <Button type="submit" disabled={isSaving} data-testid="button-save-execution-code">
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => { if (!o) setDeleteTarget(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Arkivera utförandekod?</AlertDialogTitle>
            <AlertDialogDescription>
              Utförandekoden "{deleteTarget?.label}" arkiveras. Den försvinner från valbara koder, men
              befintliga artiklar, resurser och profiler som redan använder koden behåller sin referens.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
