...
const TABS: ArchiveTabConfig[] = [
  ...
  },
  {
    key: "images",
    label: "Bilder",
    listUrl: "/api/archive/images",
    restoreUrl: (id) => `/api/archive/images/${id}/restore`,
    emptyTitle: "Inga arkiverade bilder",
    searchFields: (r) => [r.description, r.imageType, r.objectName, r.objectNumber, r.archivedReason],
    columns: [
      { header: "Beskrivning", render: (r) => <span className="font-medium">{r.description ?? r.imageType ?? "Bild"}</span> },
      { header: "Objekt", render: (r) => objectLabel(r) },
      { header: "Orsak", render: (r) => r.archivedReason ?? "—" },
      { header: "Arkiverad", render: (r) => formatDate(r.deletedAt) },
    ],
  },
  {
    key: "contacts",
    label: "Kontakter",
    listUrl: "/api/archive/contacts",
    restoreUrl: (id) => `/api/archive/contacts/${id}/restore`,
    emptyTitle: "Inga arkiverade kontakter",
    searchFields: (r) => [r.name, r.email, r.phone, r.objectName, r.objectNumber, r.archivedReason],
    columns: [
      { header: "Namn", render: (r) => <span className="font-medium">{r.name ?? "—"}</span> },
      { header: "Kontakt", render: (r) => r.email ?? r.phone ?? "—" },
      { header: "Objekt", render: (r) => objectLabel(r) },
      { header: "Arkiverad", render: (r) => formatDate(r.deletedAt) },
    ],
  },
