...
app.post("/api/mobile/customer-change-requests/confirm-photo", isMobileAuthenticated, asyncHandler(async (req: MobileAuthenticatedRequest, res: Response) => {
    ...
    const mobileTenantId = (req as any).mobileTenantId as string | undefined;
    const mobileOwner = mobileTenantId ? `tenant:${mobileTenantId}` : req.mobileResourceId;
    try {
      await oss.validateUploadedFileAndSetAcl(objectPath, mobileOwner, "private");
    } catch (err: any) {
